...
class ExecutorManager:
    """Manages multiple SQL executors for different databases.

    This class provides a centralized way to manage executors for multiple
    databases and route query execution to the appropriate executor based
    on the database name.

    Example:
        >>> executors = {
        ...     "main": SQLExecutor(pool1, security, db1),
        ...     "analytics": SQLExecutor(pool2, security, db2),
        ... }
        >>> manager = ExecutorManager(executors, default_database="main")
        >>> executor = manager.get_executor("analytics")
        >>> results, count = await executor.execute("SELECT * FROM stats")
    """
    ...
    def __init__(
        ...
        Args:
            executors: Dictionary mapping database names to SQLExecutor instances.
            default_database: Name of the default database to use when not specified.

        Raises:
            ValueError: If default_database is not in executors.
        """
        if not executors:
            raise ValueError("At least one executor must be provided")

        if default_database not in executors:
            available = list(executors.keys())
            raise ValueError(
                f"Default database '{default_database}' not found. "
    ...
    def get_executor(self, database: str | None = None) -> SQLExecutor:
        ...
        Args:
            database: Database name. If None, returns default executor.
