...
class SecurityConfig(BaseSettings):
    ...
    # Enhanced security controls (Phase 5)
    blocked_tables: list[str] = Field(
        default_factory=list,
        description="List of table name patterns to block (supports wildcards like 'audit_*')",
    )
    blocked_columns: dict[str, list[str]] = Field(
        default_factory=dict,
        description="Map of table names to blocked column lists (e.g., {'users': ['password', 'ssn']})",
    )
    allow_explain: bool = Field(
        default=True,
        description="Allow EXPLAIN statements for query analysis",
    )
    require_where_clause: list[str] = Field(
        default_factory=list,
        description="Tables that must have WHERE clause in SELECT queries",
    )
    max_join_tables: int = Field(
        default=10,
        ge=1,
        le=50,
        description="Maximum number of tables allowed in JOIN operations",
    )
    ...
    @field_validator("require_where_clause", mode="before")
    @classmethod
    def parse_require_where_clause(cls, v: str | list[str]) -> list[str]:
        """Parse comma-separated string or list of tables requiring WHERE clause."""
        if isinstance(v, str):
            return [t.strip() for t in v.split(",") if t.strip()]
        return v
