...
async def lifespan(_app: FastMCP) -> AsyncIterator[None]:  # type: ignore[type-arg]
    ...
    try:
        ...
        # SQL Executor (create one per database and wrap in ExecutorManager)
        ...
        # Create ExecutorManager for multi-database support
        executor_manager = ExecutorManager(
            executors=sql_executors,
            default_database=_settings.database.name,
        )
        logger.info(
            "ExecutorManager created",
            extra={"databases": list(sql_executors.keys()), "default": _settings.database.name},
        )
        ...
        # 8. Create QueryOrchestrator with shared resilience components
        logger.info("Creating query orchestrator...")
        _orchestrator = QueryOrchestrator(
            sql_generator=sql_generator,
            sql_validator=sql_validator,
            sql_executor=executor_manager,  # Use ExecutorManager for multi-database support
            result_validator=result_validator,
            schema_cache=_schema_cache,
            pools=_pools,
            resilience_config=_settings.resilience,
            validation_config=_settings.validation,
            circuit_breaker=_circuit_breaker,  # Inject shared circuit breaker
            rate_limiter=_rate_limiter,  # Inject shared rate limiter
        )
...
health_api = FastAPI(
    title="PG-MCP Health and Metrics",
    description="Health check and metrics endpoints for PostgreSQL MCP Server",
    version="0.1.0",
)
...
async def health_check() -> dict[str, Any]:
    ...
    """
    global _pools, _schema_cache, _orchestrator
        ...
        try:
            ...
            for db_name, pool in _pools.items():
                ...
                if pool_size == 0:
                    ...
                    break
            components["database"] = "healthy" if all_pools_healthy else "unhealthy"
        except Exception:
    ...
    # Check schema cache
    components["cache"] = "healthy" if _schema_cache is not None else "unhealthy"

    # Check orchestrator
    components["orchestrator"] = "healthy" if _orchestrator is not None else "unhealthy"
...
async def readiness_check() -> dict[str, Any]:
    ...
    """
    global _orchestrator, _pools, _schema_cache

    if _orchestrator is None:
        return {
            "status": "not_ready",
            "reason": "Service not initialized - orchestrator missing",
        }

    if not _pools or len(_pools) == 0:
        return {
            "status": "not_ready",
    ...
    if _schema_cache is None:
        return {
            "status": "not_ready",
            "reason": "Schema cache not initialized",
        }
...
async def liveness_check() -> dict[str, Any]:
    ...
    Checks if the service process is alive and running.
