...
class ExportOptions(BaseModel):
    ...
    include_headers: bool = Field(default=True, alias="includeHeaders")
    pretty_print: bool = Field(default=True, alias="prettyPrint")
    sheet_name: str = Field(default="Sheet1", alias="sheetName")
    table_name: str = Field(default="exported_data", alias="tableName")


class ExportSuggestionRequest(BaseModel):
    """Request schema for export suggestion."""
...
class ExportSuggestionResponse(BaseModel):
    """Response schema for export suggestion."""

    should_suggest: bool = Field(..., alias="shouldSuggest")
    suggested_format: str = Field(..., alias="suggestedFormat")
    reason: str
    prompt_text: str = Field(..., alias="promptText")


class NLCommandRequest(BaseModel):
    """Request schema for natural language command parsing."""

    input: str = Field(..., min_length=1)


class QueryExportEntry(BaseModel):
    """Query export history entry schema."""
...
class ExportRequest(BaseModel):
    ...
    columns: list[QueryColumn]
    rows: list[dict[str, Any]]
    format: Literal["csv", "json", "excel", "sql"] = Field(..., description="Export format")
    options: ExportOptions | None = None


class QueryAndExportRequest(BaseModel):
    """Request schema for query and export in one operation."""
