...
请先整理查看下项目，我现在需要在目前的基础上，新增一个数据导出功能模块。导出功能要求如下：
1、导出格式支持：用户可以将查询结果导出为至少两种格式（例如：CSV 文件，JSON 文件）
2、自动化流程：构建一个 Command 功能，设计一个自动化的步骤，使得“执行查询”和“导出结果”可以一键完成或通过一个简单的命令触发。
   注：可以设计一个自然语言触发或者简单的界面操作来触发导出功能（例如，在查询后，AI 助手可以主动询问：“需要将这次的查询结果导出为 CSV 或 JSON 文件吗？”）
先不要编码，构建好一个详细设计文档，命名为 0002-design-export.md，放在 @/specs 下，等我review。

### 实现

阅读 @/specs/0002-design-export.md，完成 phase 1 到 phase 3 的开发实现。
