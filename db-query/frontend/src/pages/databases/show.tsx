...
import { ExportSuggestionModal } from "../../components/ExportSuggestionModal";
...
export const DatabaseShow: React.FC = () => {
  ...
  const handleExportCSV = async () => {
    if (!queryResult || queryResult.rows.length === 0 || !id) {
  ...
  const handleExportSQL = async () => {
    ...
    try {
      ...
      message.success(`Exported ${queryResult.rowCount} rows to SQL`);
    } catch (error) {
      message.error("Export failed");
    }
  };
          ...
          <Space direction="vertical" style={{ width: "100%" }} size={24}>
            ...
            {queryResult && (
              <Card
                ...
                extra={
                  <Space>
                    <Button size="middle" onClick={handleExportCSV}>CSV</Button>
                    <Button size="middle" onClick={handleExportJSON}>JSON</Button>
                    <Button size="middle" onClick={handleExportExcel}>EXCEL</Button>
                    <Button size="middle" onClick={handleExportSQL}>SQL</Button>
                    <Button
                      size="middle"
                      type="primary"
                      icon={<DownloadOutlined />}
                      onClick={() => setExportSuggestionVisible(true)}
                    >
                      EXPORT
                    </Button>
                  </Space>
