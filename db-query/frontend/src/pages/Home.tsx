...
export const Home: React.FC = () => {
  ...
  const handleExecuteQuery = async () => {
    ...
    try {
      ...
      // Auto-show export suggestion for larger datasets
  ...
  const handleExportCSV = async () => {
    if (!queryResult || queryResult.rows.length === 0 || !selectedDatabase) {
      message.warning("No data to export");
      return;
    }

    try {
      // Use server-side export for large datasets, client-side for small ones
